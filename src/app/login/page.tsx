...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    await login('google_user@gmail.com', role, 'password123');
  };
    ...
    <div className="auth-container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 'calc(100vh - 56px - 200px)', padding: '4rem 2rem' }}>
      <div className="auth-card" style={{
        ...
        <form onSubmit={handleEmailLogin}>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--navy)' }}>
                {t.login.passwordLabel}
              </label>
              <a href="#" style={{ fontSize: '12px', color: 'var(--gold)', textDecoration: 'none' }}>{t.login.forgotPass}</a>
            </div>
            <input 
              type="password" 
              required
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '1.5rem 0' }}>
          ...
          <div style={{ padding: '0 12px', fontSize: '12px', color: 'var(--muted)', fontWeight: 500 }}>{t.login.or}</div>
          <div style={{ flex: 1, height: '1px', background: 'var(--border)' }}></div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '13px' }}>
          <span style={{ color: 'var(--muted)' }}>{t.login.noAccount} </span>
          <Link href="/register" style={{ color: 'var(--navy)', fontWeight: 600, textDecoration: 'none' }}>{t.login.signup}</Link>
        </div>
      </div>
    </div>
  );
