...
export default function UploadThesisForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleFinalSubmit = async () => {
    const thesisId = crypto.randomUUID ? crypto.randomUUID() : Date.now().toString();
    const newThesis = {
      id: thesisId,
      title,
      keywords,
    ...
    if (isSupabaseConfigured) {
      ...
      let pdfError = null;
      if (pdfFile) {
        // Upload to a separate directory/bucket or with prefix
        const { error } = await supabase.storage.from('articles_files').upload(`theses/${newThesis.id}.pdf`, pdfFile, { upsert: true });
        pdfError = error;
      }
      
      let docxError = null;
      if (docxFile) {
        const { error } = await supabase.storage.from('articles_files').upload(`theses/${newThesis.id}.docx`, docxFile, { upsert: true });
        docxError = error;
      }

      if (pdfError || docxError) {
        alert("Fayllarni yuklashda xatolik yuz berdi! Iltimos, Storage bo'limini tekshiring.");
        return;
      }

      // 2. Tezisni bazaga saqlash
      const { error } = await supabase.from('theses').insert([{ ...newThesis, id: newThesis.id }]);
      if (error) {
      ...
      const fallbackThesis = { ...newThesis, authorEmail: user?.email, id: Date.now().toString(), date: new Date().toISOString() };
      const existingStr = localStorage.getItem('gjir_theses');
      const existing = existingStr ? JSON.parse(existingStr) : [];
      localStorage.setItem('gjir_theses', JSON.stringify([fallbackThesis, ...existing]));
    }

    setIsPreviewOpen(false);
    alert("Tezisingiz muvaffaqiyatli yuborildi! Endi u Admin tomonidan tasdiqlanishi kutilmoqda.");
    ...
    <>
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '8px', padding: '2rem' }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: '14px', marginBottom: '2rem', lineHeight: '1.6' }}>
          Iltimos, tezis ma'lumotlarini to'liq kiriting. <strong>DOCX va PDF</strong> variantlarining ikkalasini ham yuklash majburiydir. 
        ...
        <form onSubmit={handleInitialSubmit}>
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={labelStyle}>Tezis mavzusi nomi *</label>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <textarea required rows={5} value={abstract} onChange={(e) => setAbstract(e.target.value)} placeholder="Tezisning qisqacha mazmunini kiriting..." style={{ ...inputStyle, resize: 'vertical' }} />
          </div>

          <div style={{ marginBottom: '1.5rem', background: 'var(--cream2)', padding: '1.5rem', borderRadius: '6px' }}>
            <label style={labelStyle}>Tezis mualliflari *</label>
